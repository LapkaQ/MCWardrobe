...
@router.post("/")
async def create_skin(name: str, db: db_dependency, skin: UploadFile = File(...), render: UploadFile = File(...)):
...
@router.get("/{skin_id}")
async def get_skin(skin_id: int, db: db_dependency):
...
async def get_skins(db:db_dependency):
    try:
